...
class ObsidianCliOps < Formula
  ...
  url "https://github.com/Data-Wise/obsidian-cli-ops/archive/refs/tags/v3.3.0.tar.gz"
  sha256 "565599d0413d1ed73769c60476d26e58f76ef5e0a2448c56b43d1cd4caca6e91"
  license "MIT"
  ...
  # Full requirements.lock closure (v3.3.0): core deps + mcp + the complete
  # transitive tree, each pinned to the version pip resolves for the lock
  # (derived via `pip install -r requirements.lock --dry-run --report`, NOT
  # `brew update-python-resources`, which resolves LATEST and pulls an
  # un-buildable attrs). A virtualenv install needs every dep listed explicitly.
  # rpds-py, pydantic-core, and cryptography are Rust sdists (see depends_on
  # "rust" => :build). To refresh after a requirements.lock change, regenerate
  # the closure the same way and reconcile versions back to the lock.
